...
{
    public partial class Frm_SH_W : Form
    {
        ...
        void Restart()
        {
            ...
            //txt_Code.Text = DateTime.Now.Date.ToString("yyyy-MM-dd") + " " + "SH_W:" + string.Format("{0:0000000}", Convert.ToInt64(DAL.ClassDAL.GetID("[Form_SH.W]", "[IDForm]")));

            //string now = DateTime.Now.Date.ToString("yy");
            //sn = Convert.ToInt64(DAL.ClassDAL.GetID("[Form_SH.W]", "[SN]"));
            //if (DAL.ClassDAL.Select("select FORMAT( max([DateForm]),'yy' ) from [Form_SH.W] ").Rows[0][0].ToString() == now)
            //{
            //    txt_SerailNumbers.Text = "و" + now + string.Format("{0:0000000}", sn);
            ...
            dt = DAL.ClassDAL.Select("select datediff(MONTH,GraduationYear, getdate() ) / 12 as YearG ,IDEng,EngName  from EngBasicData_Tbl where datediff(MONTH,GraduationYear, getdate() ) / 12>=7  ");
            ...
            DAL.Cls_SH_W.AutoCompletIssuedDepartment(txt_Department);
            DAL.Cls_SH_W.AutoCompletIssuedCity(txt_City);
            DAL.Cls_SH_W.AutoCompletIssuedGovernorate(txt_Governator);
        ...
        private void btn_Search_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(txt_SearRegistrationNo.Text))
            {
                ...
                dtSearchByRegistrationNo = DAL.ClassDAL.Select("SELECT EngName,TaxCardNo, RegistrationNo, Division, NationalID, IDEng ,EngineeringRecordNo , Specialization,ConsultantNo FROM  dbo.EngBasicData_Tbl WHERE(RegistrationNo = '" + txt_SearRegistrationNo.Text + "')");
            ...
            if (!string.IsNullOrEmpty(txt_NationalID.Text))
            {
                ...
                dtSearchByNationalID = DAL.ClassDAL.Select("SELECT EngName,TaxCardNo, RegistrationNo, Division, NationalID , IDEng,EngineeringRecordNo,Specialization ,ConsultantNo FROM  dbo.EngBasicData_Tbl WHERE(NationalID = '" + txt_NationalID.Text + "')");
            ...
            if (!string.IsNullOrEmpty(txt_searchName.Text))
            {
                ...
                dtSearchByNameEng = DAL.ClassDAL.Select("SELECT EngName,TaxCardNo, RegistrationNo, Division, NationalID , IDEng ,EngineeringRecordNo,Specialization,ConsultantNo , IDOfficeCons FROM  dbo.EngBasicData_Tbl WHERE(EngName = '" + txt_searchName.Text + "')");
            ...
            if (IDEng.Text != "" && IDOfficeCons.Text=="0")
            {
                ...
                if (!string.IsNullOrEmpty(dtCount.Rows[0]["ConsultantNo"].ToString().Trim()))
                {
                    if (Convert.ToDecimal(dtCount.Rows[0]["countD"].ToString()) < 5)
                    {
                        ...
                        #region Done    
                        GroupPayed.Enabled = groupReport.Enabled = true;
        ...
        private void txt_SearchOrderPaied_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                DataTable dt = new DataTable();
                dt = DAL.ClassDAL.Select("SELECT * FROM  dbo.Tbl_OrderPaid  where IDOrederPayed ='" + txt_SearchOrderPaied.Text + "' and TypeForm ='نموذج ش . و'");
                if (dt.Rows.Count == 0)
                {
                ...
                dt2 = DAL.ClassDAL.Select("select OrderID from [Form_SH.W] where OrderID='" + txt_SearchOrderPaied.Text + "' and State=1");
                if (dt2.Rows.Count > 0)
                {
        ...
        private void Frm_SH_W_Load(object sender, EventArgs e)
        {
            btn_New_Click(null, null);
        }
        ...
        {
            PL.Frm_ResionDelete frm = new PL.Frm_ResionDelete("SH_W", lbl_IDForm.Text, lbl_IDRev.Text, IDEng.Text, lblIDOwner.Text, txt_SerailNumbers.Text);
            frm.ShowDialog();
            btn_New_Click(null, null);
        }
        ...
        private void combEng2_SelectedIndexChanged(object sender, EventArgs e)
        {
//            SELECT IDEng, EngName, ConsultantNo, RegistrationNo, Division, Specialization
//FROM dbo.EngBasicData_Tbl where ConsultantNo != ''
        }

        private void combEng2_SelectedValueChanged(object sender, EventArgs e)
        {
        ...
        private void combEng2_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                ...
                dt_Adrs = DAL.ClassDAL.Select("SELECT Address, Phone, IDEng, IDOffice FROM dbo.Office_Tbl where IDEng = '" + lbl_IDCONENG.Text + "' ");
