...
{
    public partial class Frm_SH_D2 : Form
    {
        public decimal OrderTOPayied, FeesValue1, TaxValue1, BoxValue1;
        public Int64 sn;
        bool full = false;
        private void Frm_SH_D2_Load(object sender, EventArgs e)
        {
        ...
        //        dtSearchByNationalID = DAL.ClassDAL.Select("SELECT format(GraduationYear,'yyyy-MM') as date,TaxCardNo, EngName, RegistrationNo, Division, NationalID , IDEng,EngineeringRecordNo FROM  dbo.EngBasicData_Tbl WHERE(NationalID = '" + txt_NationalID.Text + "')");
        ...
        //        }
        //        else if (Convert.ToDecimal(dtCount.Rows[0]["YearG"].ToString()) >= 7 && Convert.ToDecimal(dtCount.Rows[0]["countD"].ToString()) < 5)
        ...
        //            txt_NoYearsExperance.Text = dtCount.Rows[0]["YearG"].ToString();
        //        }
        //        else if (dtCount.Rows[0]["ConsultantNo"].ToString() != "")
        //        {
        //            #region done
        //            DataTable dtAdressData = new DataTable();
        //            dtAdressData = DAL.ClassDAL.Select("SELECT  IDContact, Address  FROM  dbo.Contact_Tbl  WHERE   (state = 1) and IDEng='" + IDEng.Text + "'");
        ...
        private void btn_Search_Click_1(object sender, EventArgs e)
        {
            ...
            Clear(groupData);
            Clear(groupReport);
            txt_FessForm.Text = DAL.Cls_SH_D.Cost().ToString();
            groupEng.Enabled = groupOwners.Enabled = groupLisence.Enabled = btn_Edit.Enabled = groupReport.Enabled = GroupPayed.Enabled = false;
            ...
            if (IDEng.Text != "")
            {
                ...
                if (Convert.ToDecimal(dtCount.Rows[0]["YearG"].ToString()) < 7 && Convert.ToDecimal(dtCount.Rows[0]["countD"].ToString()) == 3)
                {
                    ...
                    txt_NoYearsExperance.Text = dtCount.Rows[0]["YearG"].ToString();
                    MessageBox.Show("تم اكتمال عدد الشهادات المسموح بها ", "", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    groupEng.Enabled = groupOwners.Enabled = groupLisence.Enabled = GroupPayed.Enabled = groupReport.Enabled = false;
                    full = true;
                }

               if (Convert.ToDecimal(dtCount.Rows[0]["YearG"].ToString()) >= 7 && dtCount.Rows[0]["ConsultantNo"].ToString() == ""&& Convert.ToDecimal(dtCount.Rows[0]["countD"].ToString()) < 5)
                ...
                if (full==true)
                {
                    groupEng.Enabled = groupOwners.Enabled = groupReport.Enabled = groupLisence.Enabled  = GroupPayed.Enabled = false;

                }
                else
                {
                    groupEng.Enabled = groupOwners.Enabled = groupReport.Enabled = groupLisence.Enabled  = GroupPayed.Enabled = true;
            ...
            //if (txt_NoAllowed.Text!="0")
            //{
            //    groupEng.Enabled = groupOwners.Enabled = groupReport.Enabled = groupLisence.Enabled = btn_Edit.Enabled = GroupPayed.Enabled = true;

            //}

        }

        private void txt_engName2_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                DataTable dt = new DataTable();
                dt = DAL.ClassDAL.Select("SELECT RegistrationNoCon, ConsNumber, ConsEngName  FROM  dbo.[Form_SH.D2] where ConsEngName='"+txt_engName2.Text+"'");
                txtConRegistrationNO.Text = dt.Rows[0]["RegistrationNoCon"].ToString();
                txt_conNO.Text = dt.Rows[0]["txt_conNO"].ToString();
            }
            catch (Exception ex)
            {
                return;
        ...
        {
            if (string.IsNullOrEmpty(txt_EngStamp.Text))
            {
                txt_EngStamp.Text = "0";
            }
        }
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (Convert.ToDecimal(txt_CostProject.Text) == 0)
                {
                    txt_CostProject.Focus();
                    return;
                }
                ...
                txt_Code.Text = DateTime.Now.Date.ToString("yyyy-MM-dd") + " " + "SH_D2:" + string.Format("{0:0000000}", Convert.ToInt64(DAL.ClassDAL.GetID("[Form_SH.D2]", "[IDForm]")));
                string now = DateTime.Now.Date.ToString("yy");
                sn = Convert.ToInt64(DAL.ClassDAL.GetID("[Form_SH.D2]", "[SN]"));
                if (DAL.ClassDAL.Select("select FORMAT( max([DateForm]),'yy' ) from [Form_SH.D2] ").Rows[0][0].ToString() == now)
                {
                    txt_SerailNumbers.Text = "د.ت" + now + string.Format("{0:0000000}", sn);
                ...
                txt_Area.Text = "0";
                DAL.Cls_SH_D2.Save(txt_Code.Text, Convert.ToDecimal(txt_FessForm.Text), 8, Convert.ToInt64(IDEng.Text), Convert.ToInt64(lblIDOwner.Text), txt_NumberLicense.Text, dateYear.Value.Year.ToString(), dateDateLicense.Value, txt_TypeLicense.Text, txt_IssuedFrom.Text, txt_TilteProject.Text, txt_Department.Text, txt_City.Text, txt_Governator.Text, txt_BassinesStatement.Text, txt_CostWord.Text, txt_CostProject.Text, txt_OtherData.Text, EmailEng, PhoneEng, MobileEng, AdressEng, AdressOffice, PhoneOffice, AdressOwner, EmailOwner, PhoneOwners, MobileOwner, Convert.ToInt32(DAL.ClassDAL.IDUser), TaxCardEng.Text, txt_Refrance.Text, txt_Manger.Text, txt_GeneralManager.Text, Convert.ToDecimal(txt_Area.Text), Convert.ToDecimal(txt_CostArea.Text), Convert.ToDecimal(txt_EngStamp.Text), Convert.ToDecimal(txt_StampPayied.Text), Convert.ToDecimal(txt_Rev.Text), txt_SerailNumbers.Text, Convert.ToDecimal(lbl_Fess.Text), Convert.ToDecimal(lbl_Box.Text), Convert.ToDecimal(lbl_Tax.Text), DAL.ClassDAL.MangerEngneer, sn, lbl.Text, dateTimePicker1.Value, txt_ReciptNo.Text, txt_SearchOrderPaied.Text, txt_engName2.Text, txtConRegistrationNO.Text, txt_conNO.Text);
                btn_New_Click(null, null);
        ...
        private void txt_SearchOrderPaied_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                DataTable dt = new DataTable();
                dt = DAL.ClassDAL.Select("SELECT * FROM  dbo.Tbl_OrderPaid  where IDOrederPayed ='" + txt_SearchOrderPaied.Text + "' and TypeForm ='نموذج ش . د.ت'");
                if (dt.Rows.Count == 0)
                {
                ...
                dt2 = DAL.ClassDAL.Select("select OrderID from [Form_SH.D2] where OrderID='" + txt_SearchOrderPaied.Text + "' and State=1 ");
                if (dt2.Rows.Count > 0)
                {
        ...
        {
            PL.Frm_ResionDelete frm = new PL.Frm_ResionDelete("SH_D2", lbl_IDForm.Text, lbl_IDRev.Text, IDEng.Text, lblIDOwner.Text, txt_SerailNumbers.Text);
            frm.ShowDialog();
            btn_New_Click(null, null);
        }
        ...
        void Restart()
        {
            ...
            //if (DAL.ClassDAL.Select("select FORMAT( max([DateForm]),'yy' ) from [Form_SH.D2] ").Rows[0][0].ToString() == now)
            //{
            //    txt_SerailNumbers.Text = "دت" + now + string.Format("{0:0000000}", sn);
            ...
            lbl.Text = "";
            txt_Area.Text =txt_CostProject.Text=txt_CostArea.Text= txt_StampPayied.Text = txt_EngStamp.Text = txt_Rev.Text = "0";

            txt_IssuedFrom.Enabled = true;
          
            DAL.Cls_SH_D2.AutoCompletIssuedForm(txt_IssuedFrom);
            DAL.Cls_SH_D2.AutoCompletIssuedDepartment(txt_Department);
            DAL.Cls_SH_D2.AutoCompletIssuedCity(txt_City);
            DAL.Cls_SH_D2.AutoCompletIssuedGovernorate(txt_Governator);
            DAL.Cls_SH_D2.BussinesStatement(txt_BassinesStatement);
            DAL.Clss_Owners.AutoCompletName(txt_OwnerName);

            dateTimePicker1.Value = DateTime.Now;

            combEmailOwners.Enabled = txt_NationIDOwner.Enabled =groupOwners.Enabled= txt_TilteProject.Enabled =
                txt_BassinesStatement.Enabled = txt_CostProject.Enabled = txt_CostArea.Enabled =
                txt_Rev.Enabled = txt_EngStamp.Enabled = txt_StampPayied.Enabled = txt_CostWord.Enabled = btn_SearchOwner.Enabled = checkStamp.Enabled =groupOwners.Enabled=txt_OwnerName.Enabled=combAdressOwner.Enabled=combPhoneOwners.Enabled=comb_MobileOwner.Enabled=combEmailOwners.Enabled= true;
