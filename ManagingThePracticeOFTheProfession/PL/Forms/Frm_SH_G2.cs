...
{
    public partial class Frm_SH_G2 : Form
    {

        public decimal OrderTOPayied, FeesValue1, TaxValue1, BoxValue1;
        public Int64 sn;

        private void Frm_SH_G2_Load(object sender, EventArgs e)
        {
        ...
        private void btn_New_Click(object sender, EventArgs e)
        {
            ...
            Restart();
            groupData.Enabled = groupEng.Enabled = groupLisence.Enabled = groupOwners.Enabled = GroupPayed.Enabled = groupReport.Enabled = false;
            #region deletePower
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                txt_Code.Text = DateTime.Now.Date.ToString("yyyy-MM-dd") + " " + "SH_G2:" + string.Format("{0:0000000}", Convert.ToInt64(DAL.ClassDAL.GetID("[Form_SH.G2]", "[IDForm]")));

                string now = DateTime.Now.Date.ToString("yy");
                sn = Convert.ToInt64(DAL.ClassDAL.GetID("[Form_SH.G2]", "[SN]"));
                if (DAL.ClassDAL.Select("select FORMAT( max([DateForm]),'yy' ) from [Form_SH.G2] ").Rows[0][0].ToString() == now)
                {
                    txt_SerailNumbers.Text = "ج.ت" + now + string.Format("{0:0000000}", sn);
        ...
        private void txt_SearchOrderPaied_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                dt = DAL.ClassDAL.Select("SELECT * FROM  dbo.Tbl_OrderPaid  where IDOrederPayed ='" + txt_SearchOrderPaied.Text + "' and TypeForm ='نموذج ش . ج.ت'");
                if (dt.Rows.Count == 0)
                {
                ...
                dt2 = DAL.ClassDAL.Select("select OrderID from [Form_SH.G2] where OrderID='" + txt_SearchOrderPaied.Text + "' and State=1");
                if (dt2.Rows.Count > 0)
                {
        ...
        {
            PL.Frm_ResionDelete frm = new PL.Frm_ResionDelete("SH_G2", lbl_IDForm.Text, lbl_IDRev.Text, IDEng.Text, lblIDOwner.Text, txt_SerailNumbers.Text);
            frm.ShowDialog();
            btn_New_Click(null, null);
        }
        ...
        void Restart()
        {
            ...
            //txt_Code.Text = DateTime.Now.Date.ToString("yyyy-MM-dd") + " " + "SH_G:" + string.Format("{0:0000000}", Convert.ToInt64(DAL.ClassDAL.GetID("[Form_SH.G2]", "[IDForm]")));
            //string now = DateTime.Now.Date.ToString("yy");
            //sn = Convert.ToInt64(DAL.ClassDAL.GetID("[Form_SH.G2]", "[SN]"));
            //if (DAL.ClassDAL.Select("select FORMAT( max([DateForm]),'yy' ) from [Form_SH.G2] ").Rows[0][0].ToString() == now)
            //{
            ...
            DAL.Cls_SH_G2.AutoCompletIssuedDepartment(txt_Department);
            DAL.Cls_SH_G2.AutoCompletIssuedCity(txt_City);
            DAL.Cls_SH_G2.AutoCompletIssuedGovernorate(txt_Governator);
            DAL.Clss_Owners.AutoCompletName(txt_OwnerName);
            DAL.Cls_SH_G2.BussinesStatement(txt_BassinesStatement);
