...
using clsHoorofArabic;

namespace ManagingThePracticeOFTheProfession.PL.Forms
{
    public partial class Frm_SH_D : Form
    {
        ...
        void Restart()
        {
            ...
            //txt_Code.Text =DateTime.Now.Date.ToString("yyyy-MM-dd")+" "+"SH_D:" + string.Format( "{0:0000000}"  , Convert.ToInt64(DAL.ClassDAL.GetID("[Form_SH.D]", "[IDForm]")));
            //string now = DateTime.Now.Date.ToString("yy");
            //sn = Convert.ToInt64(DAL.ClassDAL.GetID("[Form_SH.D]", "[SN]"));
            //if (DAL.ClassDAL.Select("select FORMAT( max([DateForm]),'yy' ) from [Form_SH.D] ").Rows[0][0].ToString()==now)
            //{
            //    txt_SerailNumbers.Text = "د" + now + string.Format("{0:0000000}",sn );
            ...
            txt_OtherData.Text =txt_IssuedFrom.Text= "";
            ...
            DAL.Cls_SH_D.AutoCompletIssuedDepartment(txt_Department);
            DAL.Cls_SH_D.AutoCompletIssuedCity(txt_City);
            DAL.Cls_SH_D.AutoCompletIssuedGovernorate(txt_Governator);
            ...
            txt_OwnerName.Enabled = combAdressOwner.Enabled = combPhoneOwners.Enabled = comb_MobileOwner.Enabled =
                ...
                txt_Rev.Enabled = txt_EngStamp.Enabled = txt_StampPayied.Enabled = txt_CostWord.Enabled = btn_SearchOwner.Enabled = checkStamp.Enabled = true;



        }

        private void Frm_SH_D_Load(object sender, EventArgs e)
        {
        ...
        private void groupSerchEng_Enter(object sender, EventArgs e)
        {
        ...
        private void btn_Search_Click(object sender, EventArgs e)
        {
            ...
            if (IDEng.Text != "")
            {
                ...
                if (Convert.ToDecimal(dtCount.Rows[0]["countD"].ToString()) == 5 && Convert.ToDecimal(dtCount.Rows[0]["YearG"].ToString()) < 7)// <7
                {
               ...
               if (Convert.ToDecimal(dtCount.Rows[0]["YearG"].ToString()) >= 7 && Convert.ToDecimal(dtCount.Rows[0]["countD"].ToString()) < 7)
        ...
        private void btn_OpenOwnerData_Click(object sender, EventArgs e)
        {
            //PL.Frm_Owners frm = new PL.Frm_Owners();
            //frm.ShowDialog();
        }

        private void txt_OwnerName_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            DialogResult re = MessageBox.Show("هل تريد حفظ الشهادة ","تأكيد حفظ",MessageBoxButtons.YesNo,MessageBoxIcon.Question,MessageBoxDefaultButton.Button2);
                ...
                //txt_Code.Text = DateTime.Now.Date.ToString("yyyy-MM-dd") + " " + "SH_D:" + string.Format("{0:0000000}", Convert.ToInt64(DAL.ClassDAL.GetID("[Form_SH.D]", "[IDForm]")));
                string now = DateTime.Now.Date.ToString("yy");
                sn = Convert.ToInt64(DAL.ClassDAL.GetID("[Form_SH.D]", "[SN]", DateTime.Now.Date.ToString("yyyy")));
                if (DAL.ClassDAL.Select("select FORMAT( max([DateForm]),'yy' ) from [Form_SH.D] ").Rows[0][0].ToString() == now)
                {
                    txt_SerailNumbers.Text = "د" + now + string.Format("{0:0000000}", sn);
                    txt_Code.Text = DateTime.Now.Date.ToString("yyyy-MM-dd") + " " + "SH_D:" + string.Format("{0:0000000}",sn);
        ...
        {
            PL.Frm_ResionDelete frm = new PL.Frm_ResionDelete("SH_D", lbl_IDForm.Text, lbl_IDRev.Text,IDEng.Text,lblIDOwner.Text,txt_SerailNumbers.Text);
            frm.ShowDialog();
            btn_New_Click(null, null);
            //DialogResult re = MessageBox.Show("هل تريد حذف شهادة الاشراف ؟", "إيقاف", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
            //if (re == DialogResult.Yes)
            //{

            //    DAL.Cls_SH_D.Delete(lbl_IDForm.Text, lbl_IDRev.Text);
        ...
        private void txt_StampPayied_Validating(object sender, CancelEventArgs e)
        {
        ...
        private void dateDateLicense_Validating(object er, CancelEventArgs e)
        {
          
  
        }

        private void txt_StampPayied_KeyPress(object sender, KeyPressEventArgs e)
        {
            DAL.ClassDAL.UseCurrancy(e, txt_StampPayied);
        }
        ...
        private void txt_SearchOrderPaied_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode==Keys.Enter)
            {
                ...
                dt = DAL.ClassDAL.Select("SELECT * FROM  dbo.Tbl_OrderPaid  where IDOrederPayed ='" + txt_SearchOrderPaied.Text + "' and TypeForm ='نموذج ش . د'");
                if (dt.Rows.Count==0)
                {
                ...
                dt2 = DAL.ClassDAL.Select("select OrderID from [Form_SH.D] where OrderID='" + txt_SearchOrderPaied.Text+ "' and State=1 ");
                if (dt2.Rows.Count>0)
                {
        ...
        private void txt_SearchOrderPaied_TextChanged(object sender, EventArgs e)
        {
        ...
        private void txt_SearRegistrationNo_Enter(object sender, EventArgs e)
        {
            txt_SearchOrderPaied.Text = "";
        }

        private void txt_NationalID_Enter(object sender, EventArgs e)
        {
            txt_SearchOrderPaied.Text = "";
        }

        private void txt_BassinesStatement_Enter(object sender, EventArgs e)
        {
        ...
        {
            if (string.IsNullOrEmpty(txt_CostArea.Text))
            {
                txt_CostArea.Text = "0";
            }
        }
        ...
        {
            if (string.IsNullOrEmpty(txt_Rev.Text))
            {
                txt_Rev.Text = "0";
            }
        }
