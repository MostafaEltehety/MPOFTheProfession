...
{
    public partial class Frm_SH_B : Form
    {
        ...
        void Restart()
        {
            ...
            //txt_Code.Text = DateTime.Now.Date.ToString("yyyy-MM-dd") + " " + "SH_B:" + string.Format("{0:0000000}", Convert.ToInt64(DAL.ClassDAL.GetID("[Form_SH.B]", "[IDForm]")));

            //string now = DateTime.Now.Date.ToString("yy");
            //sn = Convert.ToInt64(DAL.ClassDAL.GetID("[Form_SH.B]", "[SN]"));
            //if (DAL.ClassDAL.Select("select FORMAT( max([DateForm]),'yy' ) from [Form_SH.B] ").Rows[0][0].ToString() == now)
            //{
            ...
            DAL.Cls_SH_B.AutoCompletIssuedDepartment(txt_Department);
            DAL.Cls_SH_B.AutoCompletIssuedCity(txt_City);
            DAL.Cls_SH_B.AutoCompletIssuedGovernorate(txt_Governator);
            DAL.Clss_Owners.AutoCompletName(txt_OwnerName);
            DAL.Cls_SH_B.AutoCompletBusinesStatment(txt_BassinesStatement);
            dateTimePicker1.Value = DateTime.Now;
            DAL.Cls_SH_B.BussinesStatement(txt_BassinesStatement);
        }
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DAL.Cls_SH_B.save(txt_Code.Text, Convert.ToDecimal(txt_FessForm.Text), Convert.ToInt16(6), Convert.ToInt64(IDEng.Text), Convert.ToInt64(lblIDOwner.Text), dateTimePicker1.Value.Date, txt_NumberLicense.Text, Convert.ToDateTime(dateDateLicense.Value.Date.ToString("yyyy-MM-dd")), txt_IssuedFrom.Text, txt_TilteProject.Text, txt_Department.Text, txt_City.Text, txt_Governator.Text, txt_BassinesStatement.Text, txt_CostWord.Text, Convert.ToDecimal(txt_CostProject.Text), txt_OtherData.Text, EmailEng, PhoneEng, MobileEng, AdressEng, AdressOffice, PhoneOffice, AdressOwner, EmailOwner, PhoneOwners, MobileOwner, true, Convert.ToInt16(DAL.ClassDAL.IDUser), TaxCardEng.Text, txt_Refrance.Text, txt_Manger.Text, txt_GeneralManager.Text, txt_SerailNumbers.Text, DAL.ClassDAL.MangerEngneer, sn,IDOfficeCons.Text, Convert.ToDecimal(txt_Area.Text), Convert.ToDecimal(txt_CostArea.Text), Convert.ToDecimal(txt_EngStamp.Text), Convert.ToDecimal(txt_StampPayied.Text), Convert.ToDecimal(txt_Rev.Text), Convert.ToDecimal(lbl_Fess.Text), Convert.ToDecimal(lbl_Box.Text), Convert.ToDecimal(lbl_Tax.Text),lbl.Text,OfficeMobile,txt_ReciptNo.Text,txt_SearchOrderPaied.Text );
        ...
        private void btn_Search_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(txt_searchName.Text))
            {
                ...
                if (dtSearchByNameEng.Rows.Count > 0)
                {
                    ...
                    txt_OfficeNo.Text = dtSearchByNameEng.Rows[0]["OfficeNo"].ToString();
                }
            }
            #endregion
        ...
        private void Frm_SH_B_Load(object sender, EventArgs e)
        {
            btn_New_Click(null, null);
        }
        ...
        private void txt_NationIDOwner_VisibleChanged(object sender, EventArgs e)
        {
        ...
        private void txt_SearchOrderPaied_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                dt = DAL.ClassDAL.Select("SELECT * FROM  dbo.Tbl_OrderPaid  where IDOrederPayed ='" + txt_SearchOrderPaied.Text + "' and TypeForm ='نموذج ش . ب'");
                if (dt.Rows.Count == 0)
                {
                ...
                dt2 = DAL.ClassDAL.Select("select OrderID from [Form_SH.B] where OrderID='" + txt_SearchOrderPaied.Text + "' and State=1");
                if (dt2.Rows.Count > 0)
                {
        ...
        {
            PL.Frm_ResionDelete frm = new PL.Frm_ResionDelete("SH_B", lbl_IDForm.Text, lbl_IDRev.Text, IDEng.Text, lblIDOwner.Text, txt_SerailNumbers.Text);
            frm.ShowDialog();
            btn_New_Click(null, null);
        }
